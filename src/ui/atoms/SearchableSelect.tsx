    ...
    >
      ...
      >
        ...
        {filteredOptions.map((opt, index) => (
          <button
            key={opt.value != null ? `${String(opt.value)}-${index}` : `option-${index}`}
            type="button"
            disabled={!!opt.disabled}
            className="w-full text-left px-3 py-2 text-sm truncate text-theme-light hover:bg-[#9ff3e7]/10 cursor-pointer border-none bg-transparent disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
